...
// Grants Storage Blob Data Contributor to:
//   - workload MI (Function App)
//   - Foundry account system MI (Document Translation reads source / writes target)
param name string
...
var containers = [
  'inbound'      // uploaded source documents
  'extracted'    // Content Understanding JSON + extracted images
  'glossaries'   // per-job TSV glossary files for Document Translation
  'translated'   // Document Translation output (format-preserved)
  'reviewed'     // post-judge artifacts when REJECT
  'final'        // approved output documents
...
resource foundry_blobContributor 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(sa.id, foundryPrincipalId, roleStorageBlobDataContributor)
  scope: sa
  properties: {
