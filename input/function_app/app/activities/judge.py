"""Judge activity — Grok-3 scores translation quality across multiple dimensions.

Returns a JudgeResult with overall_score (0–5), decision (PASS/REVISE/REJECT),
and per-segment scores + issues. Guardrail issues are surfaced to the judge so
the LLM can't paper over them.
"""
...
def activity_judge(payload: dict) -> dict:
    ...
    # Normalize — accept either {overall_score, decision, per_segment:[...]} or {...per_segment:{}}
    per_segment_list = raw.get("per_segment", [])
    per_segment_map: dict[str, dict] = {}
    if isinstance(per_segment_list, list):
        for item in per_segment_list:
    ...
    elif isinstance(per_segment_list, dict):
        per_segment_map = per_segment_list
