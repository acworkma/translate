"""Glossary lookup — pull pinned translations from Cosmos and attach to segments.

Cosmos `glossary` container is partitioned by /language. Item shape:
  { id: "<source_term_lower>", language: "es", source: "...", target: "...", notes: "..." }
"""
...
glossary_lookup_bp = df.Blueprint()
...
def _lookup(language: str, terms: set[str]) -> dict[str, str]:
    ...
    lowered = sorted({t.lower() for t in terms})
    placeholders = ",".join(f"@t{i}" for i in range(len(lowered)))
    query = f"SELECT c.id, c.target FROM c WHERE c.language = @lang AND c.id IN ({placeholders})"
    params = [{"name": "@lang", "value": language}] + [
        {"name": f"@t{i}", "value": t} for i, t in enumerate(lowered)
    ]
    out: dict[str, str] = {}
    try:
        for item in container.query_items(query=query, parameters=params, partition_key=language):
...
@glossary_lookup_bp.activity_trigger(input_name="payload")
def activity_glossary_lookup(payload: dict) -> list[dict]:
